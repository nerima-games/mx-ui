...
**色は既にある。** `domain/palette.ts` がトークンと、それが満たす保証（DN-UI-11）を持っている。
**まだ無いのはその消費者**——トークンを CSS にする層——であり、
だから保証は数値については証明済みで、描かれた画面についてはまだ証明されていない。
スタイルシート・フォント・アイコンが出来た時点で publish パイプラインに影響する
（[versioning.md](./versioning.md) §4: `files` / `exports` / tsc だけでは足りないビルド）。
**トークンが `domain/` にあること自体は影響しない**——それは値であって資産ではないので、
`tsc` の型検査の対象であり、`files` の `domain` に既に含まれている。
